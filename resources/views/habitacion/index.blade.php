...
<table id="habitaciones" class="table table-hover shadow-lg mt-1">
    ...
    <tr>
      ...
      <td> @if ($habitacion->estado == 1)
        ...
        <form class="custom-control custom-switch" action="{{ route('habitacionestado', $habitacion) }}" method="post">
          ...
          <input type="checkbox" onChange="this.form.submit()" class="custom-control-input" id="customSwitch{{$habitacion->id}}" checked>
          <label class="custom-control-label" for="customSwitch{{$habitacion->id}}"></label>
          @else
          <input type="checkbox" onChange="this.form.submit()" class="custom-control-input" id="customSwitch{{$habitacion->id}}">
          <label class="custom-control-label" for="customSwitch{{$habitacion->id}}"></label>
          @endif
        </form>
      </td>
